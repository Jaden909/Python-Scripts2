...
if gameStarted:
    ...
    Turn=0
    class monsterBase:
        def __init__(self,hp,maxHp,defense,attack):
            self.hp=hp
            self.maxHp=maxHp
    ...
    class monster1(monsterBase):
        ...
        def lootDrop(self):
            ...
            if roll<=self.drop1Rate:
                self.drop1()
            if roll==self.drop2Rate:
                self.drop2()
    currentMonster=monster1(10,10,0,1)
    if speedrunMode.get():
        slashSpeed=25
    else:
        slashSpeed=50
    ...
    def encounter():
        ...
        if event=='monster1':
            ...
            Hp.set(100)
            HpStr.set(f'Hp: {currentMonster.hp}')
        if event=='wallet':
    ...
    def damage():
        ...
        damageAmt=random.choice([player.attack,player.attack+1,player.attack+2,player.attack-1])-currentMonster.defense
        if damageAmt==0:
            damageAmt=1
        ...
        if currentWeapon=='sword1' or currentWeapon=='sword3':
            SwordSlash()
        currentMonster.hp-=damageAmt
        Hp.set(currentMonster.hp*10)
        HpStr.set(f'Hp: {currentMonster.hp}')
        if currentWeapon=='sword2':
...
#__________________________________________________________________________________________________________________________________________________  
    ...
    coinsLab=Label(mainCan,textvariable=coins,background='grey75',width=1)
    butt.configure()
    butt2.configure()
    butt3.configure()
    ...
    mainCan.create_window(110,160,window=coinsLab)
