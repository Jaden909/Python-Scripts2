...
class Message:
    ...
    def send(self):
        "Triggers all objects using this message"
        for message in messages:
            if message.get('name')==self.name:
                message.update(state=True)
    def unsend(self):
        "Stops triggering objects using this message"
        for message in messages:
            if message.get('name')==self.name:
                message.update(state=False)
            
    def listen(self) -> bool: 
        "Returns whether or not this message is active"
        for message in messages:
...
class GameButton:
    ...
    def show(self,screen):
        ...
        screenn=screen
        screenn.blit(self.image,(self.x,self.y))
...
def listenPulseAll():
    ...
    for button in buttons:
        button.listenPulse()
class Vector2:
    "Tuple of xy values (WIP)"
    def __init__(self,x:int|float,y:int|float):
        self.value=(x,y)
    def translate(self,x=0,y=0):
        "Change x/y by specified amount. Negative numbers subtract."
