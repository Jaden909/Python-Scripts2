...
def startGame():
    ...
    start=time.time()
if not fastStart:
    tips=PyEngine.load('tips.json')
    tip=random.choice(tips)
    tipLines=PyEngine.autoWrap('Tip: '+tip,480,font,'white')
    mainMenuImg=pygame.image.load('mainMenu.png')
    projectImg=pygame.image.load('project.png')
    prosperImg=pygame.image.load('prosper.png')
    mainMenuOverlay=pygame.image.load('mainMenuOverlay.png')
    tipOverlay=pygame.image.load('tipOverlay.png')
    tipOverlay.set_alpha(150)
    mainMenuOverlay.set_alpha(150)
    newGameButt=PyEngine.GameButton(200,325,newGameToggle,'default',True,True,'default',64,'button.png',120,40,None,'buttonAlt.png')
    optionsButt=PyEngine.GameButton(200,380,options,'default',True,True,'default',64,'button.png',120,40,None,'buttonAlt.png')
    exitButt=PyEngine.GameButton(200,435,exit,'default',True,True,'default',64,'button.png',120,40,None,'buttonAlt.png')
    seedEntry=PyEngine.GameButton(200,60,pygame.key.start_text_input,'default',True,False,'ibeam',64,'button.png',120,40,None,None)
    equalButt=PyEngine.GameButton(60,150,lambda:setDistribution('equal'),'default',True,False,'default',64,'button.png',120,40,None,'buttonAlt.png')
    oceansButt=PyEngine.GameButton(200,150,lambda:setDistribution('oceans'),'default',True,False,'default',64,'button.png',120,40,None,'buttonAlt.png')
    balancedButt=PyEngine.GameButton(340,150,lambda:setDistribution('balanced'),'default',True,False,'default',64,'button.png',120,40,None,'buttonAlt.png')
    createWorldButt=PyEngine.GameButton(200,315,startGame,'default',True,False,'default',64,'button.png',120,40,None,'buttonAlt.png')
    line=[] 
    seedText=''
...
while True:
                ...
                if block.parentItem==getItem(currentItem['Item']):
                    ...
                    for i in range(len(obstacles)):
                        if obstacles[i].checkCollisionDamage(mouseRect,None,False,False,False):
                            ...
                            break            
    invDict=PyEngine.load('InvDebug.json')
    invList=invDict['positions']
    craftDict=PyEngine.load('craftDebug.json')
    craftList=craftDict['positions']
    hbDict=PyEngine.load('hotBarDebug.json')
    ...
    if invOpen:
        ...
        if currentItem['Item'] is not None:
            ...
            pygame.mouse.set_cursor(pygame.cursors.Cursor((8,8),pygame.image.load('hand.png')))
        if drawHitboxes:
